...
public int LegendMenuHandle(Menu menu, MenuAction action, int client, int param2)
{
	switch(action)
	{
		case MenuAction_Select:
		{
		    ...
		    if (StrEqual(info, "KS"))
			{
				Menu menu = new Menu(LegendMenuSoundHandle);

				menu.SetTitle("%s Legend Menu", g_szMenuPrefix);
				menu.AddItem("KS", "Im a fucking legend");
				menu.AddItem("M", "It s fucking bed time");
				menu.ExitButton = false;
				menu.Display(client, MENU_TIME_FOREVER);
			}
			else if (StrEqual(info, "M"))
			{
				Menu menu = new Menu(LegendMenuModelsHandle);

				menu.SetTitle("%s Legend Menu", g_szMenuPrefix);
				menu.AddItem("KS", "DeadPool");
				menu.AddItem("M", "Thanos");
				menu.ExitButton = false;
				menu.Display(client, MENU_TIME_FOREVER);
			}
		}
